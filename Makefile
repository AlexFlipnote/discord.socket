...
target:
	@echo -e "\033[1mdiscord.socket v$(shell grep -oP '(?<=__version__ = ")[^"]*' discord_socket/__init__.py)\033[0m" \
...
uninstall:  ## Uninstall the package
	pip uninstall -y discord.socket
...
flake8:  ## Run flake8 on the package
	@flake8 $(FLAKE8_CONFIG) discord_socket
	@echo -e "\033[0;32mNo errors found.\033[0m"

type:  ## Run pyright on the package
	@pyright discord_socket --pythonversion 3.11
